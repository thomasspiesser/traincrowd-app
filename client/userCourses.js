Template.userCourses.helpers({ 
  ...
  elapsedCourses: function () {
    var elapsed = Elapsed.find( { "elapsed.participants": Meteor.userId() }, {fields: {course:1, courseDate:1}} ).fetch();
    if (elapsed.length) {
  ...
  myRating: function (id) {
    var elapsed = Elapsed.findOne( {_id: id }, {fields: {_id:0, myRating:1}} );

    var myRating = _.find(elapsed, function (item) { 
