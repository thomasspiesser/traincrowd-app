Meteor.methods({
  redeemCoupon( options ) {
    ...
    let fields = { customer: 1, bookingStatus: 1, eventId: 1 };
    let booking = Bookings.findOne( { _id: options.bookingId }, { fields: fields } );
    ...
    let current = Current.findOne( { _id: booking.eventId }, { fields: { coupons: 1 } } );
    if ( ! current.coupons ) {
    ...
    let coupon = _.find( current.coupons, function( coupon ) { return coupon.code === options.code; } );
  ...
  createEvent(options) {
    ...
    let fields = { owner: 1, title: 1 };
    let course = Courses.findOne({_id: options.courseId}, {fields: fields });
  ...
  deleteEvent(options) {
    ...
    fields = { owner: 1 };
    let current = Current.findOne( { _id: options.currentId }, { fields: fields } );
  ...
  declineEvent( token ) {
    ...
    let fields = { owner: 1, course: 1, participants: 1, courseDate: 1 };
    let current = Current.findOne( { token: token }, { fields: fields } );
    checkExistance( current, 'Event', fields );
  ...
  updateRoles() {
    ...
    let slug = slugify( Meteor.users.findOne( this.userId ).getName() );
  ...
  rateCourse( options ) {
    ...
    if ( this.userId ) {
      ...
      let user = Meteor.users.findOne( { rateTokens: options.token }, { fields: { _id: 1 } } );
      if ( ! user ) {
    ...
    let fields = { course: 1, participants: 1, ratings: 1, owner: 1 };
    let elapsed = Elapsed.findOne( { _id: options.elapsedId }, { fields: fields } );
    ...
    let existingRating = _.find( elapsed.ratings, function(item) { return item.participant === userId; } );
    if ( existingRating ) {
