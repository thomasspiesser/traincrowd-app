...
Courses.helpers({
  ...
  getFeePP() {
    return parseInt( this.fee / this.maxParticipants, 10 );
  },
  getImage() {
    if ( this.imageId ) return this.imageId;
    let trainer = Meteor.users.findOne( { _id: this.owner }, {
  ...
  getNextEvent() {
    let id = this.__originalId || this._id; // this is the fix for easy:search
    let currents = Current.find( { course: id }, {
      sort: { courseDate: 1 },
      fields: { participants: 1, courseDate: 1, confirmed: 1 },
    }).fetch();
    let current = _.find( currents, ( event ) => {
      return event.participants.length !== this.maxParticipants;
    });
