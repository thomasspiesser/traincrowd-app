...
Meteor.methods({
  ...
  createBooking: function (currentId, courseId) {
    ...
    var current = Current.findOne( { _id: currentId }, { fields: { course: 1, courseDate: 1 } } );
    ...
    var course = Courses.findOne( { _id: courseId }, { fields: { title: 1, fee: 1, owner: 1, ownerName: 1, maxParticipants: 1 } } );
    ...
    if ( ! course.fee )
      throw new Meteor.Error(403, "Kurspreis nicht gefunden");

    if ( ! course.maxParticipants )
      throw new Meteor.Error(403, "max Kursteilnehmerzahl nicht gefunden");

    if ( ! course.owner )
      throw new Meteor.Error(403, "Kurs-Besitzer nicht gefunden");

    if ( ! course.ownerName )
      throw new Meteor.Error(403, "Kurs-Besitzername nicht gefunden");

    var commision = calcCommision( course.fee );
    var courseFeePP = +( ( course.fee + commision ) / course.maxParticipants ).toFixed(2);
  ...
  updateBooking: function (args) {
    ...
    var booking = Bookings.findOne( { _id: args.bookingId }, { fields: { customer: 1 } } );
