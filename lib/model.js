...
Meteor.methods({
  ...
  updateUserSingleField: function (args) {
    check(args, {
      argName: String,
      argValue: Match.OneOf(String)
      // argValue: Match.OneOf(String, Boolean, Number, [String])
    });
    ...
    if ( args.argName === 'email' ) {
      ...
      // catch validation error - return to client
      return;
  ...
  updateUser: function (options) {
    ...
    if (options['profile.name']) {
      ...
      Courses.update({owner: this.userId}, { $set: { ownerName: options['profile.name'] } }, { multi: true } );
      // same for Currents
      Current.update({owner: this.userId}, { $set: { ownerName: options['profile.name'] } }, { multi: true } );
    }
