...
Meteor.methods({
  ...
  rateCourse: function (options) {
    ...
    if (! this.userId)
      throw new Meteor.Error(403, "Sie müssen eingelogged sein!");
    var elapsed = Elapsed.findOne({_id: options._id}, {fields: {course:1, participants:1, ratings:1}});
    if (! elapsed)
    ...
    Meteor.call('updateCourseRating',elapsed.course);
  },
  updateCourseRating: function (course) {
    ...
    Courses.update(course, {$set: {rating: avgRating} });
  },
