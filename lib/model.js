...
/*
  Each course is represented by a document in the Courses collection:
    ...
    proposed: Array of objects like {instanceId: id, proposer: userId, proposedDates: [dates], createdAt: currentDate } createdAt is important, so proposal can expire
...
proposeCourseDates = function (options) {
  var id = Random.id();
  Meteor.call('proposeCourseDates', _.extend({ _id: id }, options));
  return id;
};
...
Meteor.methods({
  ...
  confirmCourseDate: function (courseId, instanceId, confirmedDate) {
    // check that user._id = course.owner
    var proposer = Courses.findOne( {_id: courseId}, { _id: 0, 'proposed.proposer': 1} );
    Courses.update(
      courseId, 
      { $push: {current:
        { instanceId: instanceId, 
        participants: [proposer], 
        courseDate: confirmedDate }
              }
      }
    );
  },
